(function(){
  ...
  function render(){
    ...
    host.setAttribute('aria-label','主要功能選單');
  ...
  function scriptAlreadyLoaded(src){
    return Array.from(document.scripts).some(script=>{
      ...
      return value===src||value.endsWith('/'+src)||value.endsWith(src);
    });
  }
  ...
  async function bootstrap(){
    try{
      ...
      await loadScript('js/services/supabaseServiceV121.js');
      if(page==='index'){
        await loadScript('js/modules/scheduleV120.js');
        await loadScript('js/modules/scheduleV121.js');
      }
    }catch(error){
      console.error('[v1.2.1 bootstrap]',error);
    }
  }

  render();
  if(!document.getElementById('v121-menu-style')){
    const style=document.createElement('style');
    style.id='v121-menu-style';
